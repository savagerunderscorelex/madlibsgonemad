...
9:59 PM I'm nearly done: I did the nav, footer, and background design for nearly all pages except the link page.  I had to replace the ids and classes since I decided last second to change the colors of SOME pages (remind me to plan better next time) I added the images for mad libs 2 and 3's background, so I had to change a lot of the style code. It took me sooooo long to also fix the issue with the select mad lib page buttons, since the 3000 port showed the buttons fine but neither the github page or the 5000 port showed them well.
